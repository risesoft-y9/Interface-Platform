...
//			authDictService.copyAuthDictListByInterfaceId(id,newId);
//		}
        map.put("code", "0");
        map.put("data", newId);
        return map;
    ...
    //保存接口文档
    @RequestMapping("/saveInterfaceFile")
    @ResponseBody
    @RiseLog(operationType = OperationTypeEnum.ADD, operationName = "保存接口文档")
    public Map<String, Object> saveInterfaceFile(MultipartFile file) {
        Map<String, Object> map = new HashMap<>();
        map.putAll(interfaceManageService.saveInterfaceFile(file));
        map.put("code", "0");
        return map;
    }
    ...
    public Map<String, Object> getApplyInfoByInterfaceId(String id) {
        ...
        if (StringUtils.isNotBlank(applyInfo.getUserSecret())) {
            String secret = applyInfo.getUserSecret().substring(0, 4) + "******" + applyInfo.getUserSecret().substring(applyInfo.getUserSecret().length() - 5, applyInfo.getUserSecret().length());
