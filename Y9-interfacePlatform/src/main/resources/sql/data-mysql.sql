-- 删除表
DROP TABLE y9_public.view_approve_list;
--DROP TABLE IF EXISTS view_approve_list;

-- 创建视图
create or replace
algorithm = UNDEFINED view `y9_public`.`view_approve_list` as
select
...
from
    ((`y9_public`.`y9_interface_approve` `nia`
        left join `y9_public`.`y9_interface_apply` `nia2` on
        ((`nia`.`APPLY_ID` = `nia2`.`ID`)))
        left join `y9_public`.`y9_interface_manage_info` `nimi` on
