    ...
    <y9Dialog v-model:config="addDialogConfig">
        <template v-slot>
            <y9Form ref="ruleFormRef" :config="ruleFormConfig">
                ...
                <template #interfaceFile>
                    <el-button @click="viewFile">预览接口文档</el-button>
                </template>
            </y9Form>
        </template>
    </y9Dialog>
        ...
        return new Promise(async (resolve, reject) => {
            ...
            await y9RuleFormInstance.validate(async (valid) => {
                if (valid) {
                    ...
                    let parameterIds = []
                    if (value2.value == 'true' || value2.value) {
...
async function addDialog() {
    ...
    addDialogConfig.value.title = computed(() => t('新增接口信息'))
...
//详情按钮
async function approveView(id, show) {
    initFormData()
    let para = {
        id: id
    }
    ...
    for (let it of ruleFormConfig.value.itemList) {
        ...
        arry.push(it)
        if(it.prop == "illustrate"){
            let interfaceFileUrl = {
                type: 'slot',
                label: computed(() => t('接口文档')),
                prop: 'interfaceFile',
                props: {
                    slotName: "interfaceFile"
                }
            }
...
//接口文档预览
const viewFile = ()=>{
    downLoadInterfaceFile(ruleFormConfig.value.model.interfaceFileUrl).then((res) => {
        const binaryData = [];
        binaryData.push(res);
...
defineExpose({ addDialog, edit, view, updateVersion,approveView })
</script>
<style>
.leftMargin {
    margin-left: 5px;
